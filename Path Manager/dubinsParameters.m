...
%     theta = atan2(cre(2)-crs(2),cre(1)-crs(1));
    theta = -(atan2(cre(1)-crs(1),cre(2)-crs(2)) - pi/2);
    L1 = norm(crs-cre)+...
...
%     theta = atan2(cle(2)-crs(2),cle(1)-crs(1));
    theta = -(atan2(cle(1)-crs(1),cle(2)-crs(2)) - pi/2);
...
%     theta = atan2(cre(2)-cls(2),cre(1)-cls(1));
    theta = -(atan2(cre(1)-cls(1),cre(2)-cls(2)) - pi/2);
    theta2 = acos(2*R/ell);
    if isreal(theta2)==0,
      L3 = 9999;
    else
...
%     theta = atan2(cle(2)-cls(2),cle(1)-cls(1));
    theta = -(atan2(cle(1)-cls(1),cle(2)-cls(2)) - pi/2);
    L4 = norm(cls-cle)+...
    ...
    switch(idx),
        ...
        case 2,   
            ...
            lame = -1;
            ell = norm(ce-cs);
%             theta = atan(ce(2)-cs(2),ce(1)-cs(1));
            theta = -(atan2(ce(1)-cs(1),ce(2)-cs(2)) - pi/2);
            theta2 = theta-pi/2+asin(2*R/ell);
